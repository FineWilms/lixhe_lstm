...
model = Sequential()
print(n_steps)
model.add(LSTM(100, activation='relu', batch_input_shape=(batchsize, n_steps, n_features), stateful=True))
model.add(Dense(1))
...
history = model.fit(X, y, epochs=1000, verbose=1, shuffle=False, batch_size=batchsize, validation_data=(Xval, yval), callbacks=[es])
